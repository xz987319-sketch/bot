...
⚠️ 【注意事项】
• /add、/list 仅管理员可使用，普通用户仅能查询；
• 添加账户时，务必用Ctrl+Enter换行（不是Enter发送）；
• 群聊查询需先将机器人设为群管理员（勾选「读取消息+发送消息」）。
...
def add_admin(update: Update, context: CallbackContext):
    ...
    try:
        ...
        msg = f"✅ 管理员「{admin_id}」添加成功！" if c.rowcount > 0 else f"❌ ID「{admin_id}」已是管理员！"
        conn.commit()
        conn.close()
        update.message.reply_text(msg)
    except ValueError:
...
def remove_admin(update: Update, context: CallbackContext):
    ...
    try:
        ...
        msg = f"✅ 管理员「{admin_id}」移除成功！" if c.rowcount > 0 else f"❌ ID「{admin_id}」不是管理员！"
        conn.commit()
        conn.close()
        update.message.reply_text(msg)
    except ValueError:
